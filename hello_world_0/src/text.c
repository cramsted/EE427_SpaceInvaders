...
const int32_t *digitsArray[] = { zero_8x5, one_8x5, two_8x5, three_8x5, four_8x5,
...
void drawCharacters() {
	...
	const int32_t *scoreArray[] = { s_8x5, c_8x5, o_8x5, r_8x5, e_8x5 };
	int32_t i;
...
void drawGameOver() {
	const int32_t *gameArray[] = { g_8x5, a_8x5, m_8x5, e_8x5 };
	int32_t i;
	for (i = 0; i < MAX_CHARACTER_LETTERS - 1; i++) {
		Character s = initChar(
				(GAME_START_X + (CHARACTER_OFFSET) * i),
				GAME_OVER_Y, gameArray[i], RED);
		editFrameBuffer(&s.sp, &s.p);
	}
	const int32_t *overArray[] = { o_8x5, v_8x5, e_8x5, r_8x5 };
	for (i = 0; i < MAX_CHARACTER_LETTERS - 1; i++) {
		Character s = initChar(
				(OVER_START_X + (CHARACTER_OFFSET) * i),
...
void initScore() {
	...
	for (i = 0; i < MAX_SCORE_DIGITS; i++) {
		score.sp[i] = initChar(
				DIGITS_START_X + (i * (CHARACTER_OFFSET)),
...
void drawScore() {
	int32_t digit_one = (score.score / 1000) % 10;
	int32_t digit_two = (score.score / 100) % 10;
	int32_t digit_three = (score.score / 10) % 10;
	int32_t digit_four = 0;
...
void drawUfoPoints(Position *p, int32_t points, uint32_t color) {
	int32_t digit_one = (points / 100) % 10;
	int32_t digit_two = (points / 10) % 10;
	int32_t digit_three = 0;
